...
class BSTable : Table<Tkey, Tdata> { //  Table<Tkey, TabRecord<Tkey, Tdata>*> ??
private:
...
TabRecord<Tkey, Tdata>* BSTable<Tkey, Tdata>::Find(Tkey k) {
    TreeNode<Tkey, TabRecord<Tkey,Tdata>*>* tr = this->bst.Search(k);
    return *tr->data;
}
...
void BSTable<Tkey, Tdata>::Insert(TabRecord<Tkey, Tdata>* tr) {
    ...
    TreeNode<Tkey, TabRecord<Tkey, Tdata>*>* rec = new TreeNode<Tkey,TabRecord<Tkey,Tdata>*>(tr->get_key(), &tr);
    this->Insert(rec);
    this->count++;
}
